...
export default function NewQuotePage() {
  ...
  // Auto-generate quote when landing with trip_id (e.g. from intake flow)
  useEffect(() => {
    if (!tripIdParam || autoGeneratedRef.current) return;
  ...
  // Auto-run route plan when trip + aircraft are selected (AI-driven, no manual click)
  useEffect(() => {
    if (!selectedTripId || !selectedAircraftId) return;
    const trip = trips.find((t) => t.id === selectedTripId);
    if (!trip || trip.legs.length === 0) return;
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? "Failed to create quote");
      const quoteId = data.quote?.id;
      // Agent already computes and persists the route plan via save_route_plan
  ...
  const canSave = !!selectedTripId;

  // Auto-generating: show loading state (e.g. from intake with ?trip_id=)
  if (autoGenerating) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-zinc-500">
            AI is selecting the best aircraft and route plan.
          </p>
        </div>
      </div>
    );
    ...
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-zinc-600">
          {tripIdParam
            ? "Building quote automatically. If it failed, select options below."
